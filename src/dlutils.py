...
# 自定义TransformerDecoder，解决PyTorch版本兼容性问题
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None, tgt_key_padding_mask=None,
                memory_key_padding_mask=None, is_causal=None):
        # 忽略is_causal参数，保持与旧版本的PyTorch兼容性
        output = tgt
...
class Cholesky(torch.autograd.Function):
    ...
    # 自定义TransformerEncoder，解决PyTorch版本兼容性问题
...
class LinearAnomalyAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        if self.output_attention:
            # 修改这里：将queries和keys合并为一个注意力矩阵
            attn = torch.einsum("blhe,bshe->bhls", queries, keys)
...
class SparseAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        参数:
        queries: 形状为 [batch_size, seq_len, num_heads, head_dim]
        keys: 形状为 [batch_size, seq_len, num_heads, head_dim]
        values: 形状为 [batch_size, seq_len, num_heads, head_dim]
        mask: 可选的掩码张量
        
        返回:
        output: 形状为 [batch_size, seq_len, num_heads, head_dim]
        attention: 注意力分数，形状为 [batch_size, num_heads, seq_len, seq_len]
        """
        B, L, H, E = queries.shape
        assert L % self.block_size == 0, "序列长度应该被块大小整除"
        ...
        for i in range(0, L, block_size):
            block_mask[i:i + block_size, i:i + block_size] = True
            if i + block_size < L:
                block_mask[i:i + block_size, i + block_size:i + 2 * block_size] = True  # 块与下一个块
                block_mask[i + block_size:i + 2 * block_size, i:i + block_size] = True  # 下一个块与当前块
        if mask is not None:
...
# 基于块稀疏注意力的Transformer编码器层
class SparseTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, block_size, dim_feedforward=16, dropout=0):
        ...
        # 块稀疏注意力
        self.sparse_attention = SparseAttention(block_size, nhead)
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        # 转换为 [batch_size, seq_len, d_model] 便于处理
        src_t = src.transpose(0, 1)
        
        # 投影并重塑为多头格式 [batch_size, seq_len, nhead, head_dim]
        q = self.query_proj(src_t).view(batch_size, seq_len, self.nhead, self.head_dim)
        k = self.key_proj(src_t).view(batch_size, seq_len, self.nhead, self.head_dim)
        v = self.value_proj(src_t).view(batch_size, seq_len, self.nhead, self.head_dim)
        
        # 应用稀疏注意力
        attn_output, self.att = self.sparse_attention(q, k, v, src_mask)
        
        # 合并多头输出 [batch_size, seq_len, d_model]
        attn_output = attn_output.reshape(batch_size, seq_len, self.d_model)
        attn_output = self.output_proj(attn_output)
        
        # 转回原始形状 [seq_len, batch_size, d_model]
...
# 基于线性注意力的Transformer编码器层
class LinearTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=16, dropout=0, mapping_fun='ours'):
        ...
        # 线性注意力
        self.linear_attention = LinearAnomalyAttention(
            win_size=10,  # 假设窗口大小为10，可根据实际情况调整
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        # 应用线性注意力
        attn_output, self.att = self.linear_attention(q, k, v)
